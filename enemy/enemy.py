...
class Slime:
    ...
    def main(self, display, bullet_list):
        ...
        if self.health >= 3:
            ...
            if self.base_x + self.offset_x > self.x - self.display_scroll[0]:
                self.x += 1
            elif self.base_x + self.offset_x < self.x - self.display_scroll[0]:
                self.x -= 1

            if self.base_y + self.offset_y > self.y - self.display_scroll[1]:
                self.y += 1
            elif self.base_y + self.offset_y < self.y - self.display_scroll[1]:
                self.y -= 1
        else:
